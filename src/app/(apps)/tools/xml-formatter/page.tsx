...
export default function XMLFormatterPage() {
  ...
  const handleFormat = () => {
    ...
    setTimeout(() => {
      // Basic client-side XML formatter regex pipeline
      let formatted = ''
      let indent = ''
      const tab = '  '

      // Split tags
      const tokens = xml.replace(/>\s*</g, '><').trim().split(/(?=<)/)
  ...
  const handleLoadSample = () => {
    setXml("<note><to>Tove</to><from>Jani</from><heading>Reminder</heading><body>Don't forget me this weekend!</body></note>")
  }
    ...
    <div className="space-y-8">
      ...
      <Section delay={0.05}>
        <div className="space-y-2">
          ...
          <GradientHeading as="h1" className="text-3xl sm:text-4xl">XML Formatter</GradientHeading>
          <p className="text-muted-foreground text-xs sm:text-sm">
            Beautify, indent, and format nested XML markup tags structure safely.
          </p>
        </div>
      </Section>
            ...
            <Card className="flex flex-col h-[280px]">
              ...
              <CardContent className="flex-1 p-0">
                <textarea
                  ...
                  placeholder="Paste your unformatted XML markup here..."
          ...
          <div className="lg:col-span-7">
            {result ? (
              ...
              <div className="border border-dashed border-border/60 rounded-xl h-[340px] flex flex-col items-center justify-center text-center text-muted-foreground space-y-2 p-6">
                ...
                <p className="text-sm font-semibold text-foreground/80">Pending XML Input</p>
                <p className="text-xs max-w-sm">
                  Paste unformatted XML content on the left to structure and align formatting columns.
                </p>
              </div>
            )}
          </div>
